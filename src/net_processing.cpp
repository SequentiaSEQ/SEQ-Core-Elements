...
void PeerManagerImpl::MaybeSendFeefilter(CNode& pto, std::chrono::microseconds current_time)
{
    ...
    CAmount currentFilter = m_mempool.GetMinFee(gArgs.GetIntArg("-maxmempool", DEFAULT_MAX_MEMPOOL_SIZE) * 1000000).GetFeePerK().value;
    ...
    if (current_time > pto.m_tx_relay->m_next_send_feefilter) {
        ...
        filterToSend = std::max(filterToSend, ::minRelayTxFee.GetFeePerK().value);
        ...
        if (pto->m_tx_relay != nullptr) {
                ...
                if (fSendTrickle && pto->m_tx_relay->fSendMempool) {
                    ...
                    for (const auto& txinfo : vtxinfo) {
                        ...
                        if (txinfo.fee < filterrate.GetFee(txinfo.vsize).value) {
                            continue;
                        }
